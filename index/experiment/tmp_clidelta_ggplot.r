...
#
p <- p + geom_bar(aes(y=r,fill=c),stat = "identity",colour="black") # need identity to draw value itself.
p <- p + geom_point(aes(y=i,colour=c),stat="identity", position="identity",size=0.8)
