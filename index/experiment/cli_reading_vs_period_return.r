#
# 1)pick up months whose clie_xts$oecd  is more than 100
# 2)create the stream of flags, in which up is 1 and down is 0
# 3)compare SPX close price between the end month of cli_xts is more(less) than 100 and its start.
...
# put flag on the months accoring to the parameter. for "minus" cli delta is less than ZERO, for plus the opposite.
  for(i in seq(1,length(cli_xts$oecd[period]),1,)){
    if(plus_or_minus == "minus"){
      if(as.vector(cli_xts$oecd[period])[i] < 100){  # up is "> 0"
        w <- append(w,1)
      }else{
        w <- append(w,0)
      }
...
# check stream and when flag is changes 0 to 1. it is the start of period.
  for(i in seq(1,length(cli_xts$oecd[period]),1,)){
    if(w[i] == 1){
      ...
      if(i == length(cli_xts$oecd[period])){
...
rbind(merge(wp,rep(1,12),suffixes =c('','p_or_m')),merge(wm,rep(-1,13)))
