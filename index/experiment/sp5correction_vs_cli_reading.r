...
# this program classifies months into 4 groups. 
#   1) cli is more than 100 and delta is plus as "dp"
#   2) cli is more than 100 and delta is minus as "pm"
#   3) less than 100 and plus as "mp"
#   4) less than 100 and minus as "mm"
#
func <- function(delta=0.9,m="c",l=5){
  lag_month <- l
  ind_bp <- index(na.omit(diff(cli_xts$oecd,lag=lag_month))["1962::"])[cli_xts$oecd["1962::"] >100 & na.omit(diff(cli_xts$oecd,lag=1))["1962::"] > 0]


  ind_bm <- index(na.omit(diff(cli_xts$oecd,lag=lag_month))["1962::"])[cli_xts$oecd["1962::"]  <100  & na.omit(diff(cli_xts$oecd,lag=1))["1962::"] < 0]

  ind_mp <- index(na.omit(diff(cli_xts$oecd,lag=lag_month))["1962::"])[cli_xts$oecd["1962::"]  <100  & na.omit(diff(cli_xts$oecd,lag=1))["1962::"] > 0]

  ind_pm <- index(na.omit(diff(cli_xts$oecd,lag=lag_month))["1962::"])[cli_xts$oecd["1962::"] >100  & na.omit(diff(cli_xts$oecd,lag=1))["1962::"] < 0]
