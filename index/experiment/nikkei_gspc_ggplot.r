...
cbind(apply.monthly(na.omit(N225[k3]), mean, na.rm = T), as.vector(apply.monthly(SP5[k3][, 4], mean, na.rm = T)), as.vector(apply.monthly(na.omit(YJUSDJPY)[k3][, 4], mean, na.rm = T))) -> lmdf # edit
head(lmdf)
...
df <- data.frame(
  i = as.vector(apply.monthly(na.omit(N225[k3]), mean)), # edit
  ...
  t = as.Date(index(apply.monthly(na.omit(N225[k3]), mean))) # edit
...
p <- p + annotate("text", label = output.label, x = as.Date(START_DATE), y = max(as.vector(to.monthly(N225[k3])[, 4])) * 0.9, hjust = 0) # edit
