...
# VIX <- as.xts(read.zoo(read.csv("~/VIX.csv")))
mnt4 <- index(cli_xts$oecd[start_date][na.omit(diff(cli_xts$oecd,lag=lag_month))[period] < 0 & na.omit(diff(cli_xts$oecd))[period] < 0])
# plot.zoo(merge(VIX["2000::2019-02"][,4],VIX[mnt4][,4]),type='h',col = c("red", "blue"), plot.type = "single")
plot.zoo(merge(VIX[paste(start_date,substr(as.character(index(last(cli_xts))),1,7),sep="")][,4],VIX[mnt4][,4]),type='h',col = c("red", "blue"), plot.type = "single")
